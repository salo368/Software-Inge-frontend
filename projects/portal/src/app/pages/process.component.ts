...
import { SignaturesService } from '@shared/core/signatures.service';
...
export class ProcessComponent implements OnInit {
  ...
  startSignature(): void {
    ...
    if (pending && pending.stage !== 'signed') {
      this.goToSigning(pending.token);
      return;
    }
    ...
    this.signaturesApi.create(d.process.id).subscribe({
      next: (r) => {
        ...
        this.goToSigning(r.signature.token);
      },
      error: (err) => {
        this.signing.set(false);
        this.error.set(this.friendlyError(err?.error?.error));
  ...
  /** The signing ceremony is its own block, so this leaves the portal. */
  private goToSigning(token: string): void {
    window.location.assign(blockUrl('signing', token));
  }
